...
export function Sidebar() {
  ...
  // The parent normally prevents this state; returning null makes the component
  // safe if rendered independently during auth transitions or tests.
  if (!currentUser) {
    return null;
  }

  // Remove destinations that do not list the current account role.
  const navigation = filterNavigationForUser(SIDEBAR_NAVIGATION, currentUser);

  return (
    <>
      {/* Tablet/desktop navigation remains fixed-width beside page content. */}
      <aside className="hidden w-48 shrink-0 bg-[#160926] p-4 md:flex md:flex-col shadow-xl z-10">
        ...
        <nav className="space-y-2">
          {navigation.map((item) => {
            // Exact matching highlights top-level destinations.
      ...
      {/* Mobile navigation is fixed above the viewport bottom/player region. */}
        ...
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              ...
              {/* A small glowing dot reinforces the active item on narrow screens. */}
              {active ? (
