...
export default function AlbumPage({ params }: AlbumPageProps) {
  ...
  if (loading) return <MainAppLayout>{t("Loading album...")}</MainAppLayout>;
      ...
      {album ? (
        <>
          <div className="mb-8 mt-4 flex flex-col items-center gap-6 sm:flex-row sm:items-end">
            {album.coverImageUrl ? <img alt={album.title} className="h-48 w-48 rounded-md object-cover shadow-2xl" src={album.coverImageUrl} /> : <div className="flex h-48 w-48 items-center justify-center rounded-md bg-surface-800">{t("No cover")}</div>}
            <div className="flex flex-col gap-3 text-center sm:text-start">
              <span className="text-xs font-bold uppercase tracking-widest text-slate-400">{t("Album")}</span>
              ...
              <div className="flex items-center justify-center gap-2 text-sm text-slate-300 sm:justify-start">
                <Link className="font-semibold hover:underline" href={`/artist/${album.artistId}`}>{album.artistName ?? t("Unknown artist")}</Link>
                <span>•</span><span>{new Intl.NumberFormat(locale, { useGrouping: false }).format(new Date(album.releaseDate).getFullYear())}</span><span>•</span><span>{t("{count} tracks", { count: new Intl.NumberFormat(locale).format(albumTracks.length) })}</span>
              </div>
            </div>
          </div>
          <div className="mb-8 flex gap-4">
            <Button disabled={queueTrackIds.length === 0} onClick={() => void playAlbumFrom(queueTrackIds[0])} size="lg">{t("Play")}</Button>
            <Button disabled={queueTrackIds.length === 0} onClick={handleShuffle} size="lg" variant="secondary">{t("Shuffle")}</Button>
          </div>
          <section className="grid gap-2">
            {albumTracks.map((track, index) => (
              <div className="flex items-center gap-4" key={track.id}>
                <div className="w-8 text-center text-slate-400">{new Intl.NumberFormat(locale).format(index + 1)}</div>
        ...
        <EmptyState action={<Button onClick={() => router.push("/music")} variant="secondary">{t("Back to music")}</Button>} description={error || t("The requested album does not exist.")} title={t("Album not found")} />
      )}
    </MainAppLayout>
  );
}
