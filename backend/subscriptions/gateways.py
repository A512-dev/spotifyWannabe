...
def get_gateway(name: str | None = None):
    selected = (name or os.getenv("PAYMENT_GATEWAY", "local")).strip().lower()
    if selected in {"local", "local-sandbox"}:
        return LocalSandboxGateway()
    if selected == "zarinpal":
        return ZarinpalGateway()
    raise PaymentGatewayError(f"Unsupported payment gateway: {selected}")
