...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_dailyStreamCount(self, obj) -> int:
        stream_events = getattr(obj, "stream_events", None)
        if stream_events is None:
            return 0

        from django.utils import timezone

        return stream_events.filter(
            counted=True,
...
class PasswordResetConfirmSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        except (BinasciiError, UnicodeDecodeError, ValueError, TypeError, OverflowError, User.DoesNotExist):
